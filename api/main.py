    ...
    try:
        ...
        for sym in assets:
            try:
                ...
                # Prodloužená pauza pro stabilitu Free Tieru
                time.sleep(3) 
                ...
                for tf in ["1M", "15M", "1H", "1D"]:
                    time.sleep(3) # Zde je kritické zpomalení na 3 vteřiny
                ...
                print(f"Chyba u {sym}: {inner_e}")
                time.sleep(5) # Při chybě (včetně 429) počkáme déle
                continue

        return ("OK - Terminal v41.1 (Stable Mode)", 200, headers)

    except Exception as e:
        return (f"Error: {str(e)}", 200, headers)
